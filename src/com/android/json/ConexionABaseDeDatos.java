...
public class ConexionABaseDeDatos extends SQLiteOpenHelper {
	...
	private String crearPrograma = "CREATE TABLE `programa` ( `idprogramas` INT NOT NULL , `nombre` VARCHAR(60) NULL, `horaDeInicio` VARCHAR(60) NULL , `canales_idcanales` INT NOT NULL , PRIMARY KEY (`idprogramas`,`canales_idcanales`), FOREIGN KEY (`canales_idcanales` ) REFERENCES `canal` (`idcanales` ));";
	...
	// insertar Programas
	public void insertarPrograma(int id, String nombre, String horaDeInicio,
			int idCanal) {
		ContentValues valores = new ContentValues();
	...
	public String leerPrograma(int idCanal) {
		...
		String columna[] = { "nombre", "horaDeInicio", "canales_idcanales" };
		Cursor c = this.getReadableDatabase().query("programa", columna, null,
				null, null, null, null);
		int indexNombre = c.getColumnIndex(columna[0]);
		int indexHoraDeInicio = c.getColumnIndex(columna[1]);
		int indexcanales_idcanales = c.getColumnIndex(columna[2]);
		while (c.moveToNext()) {
			if (idCanal == c.getInt(indexcanales_idcanales)) {
				resultado = resultado + c.getString(indexHoraDeInicio) + " - "
						+ c.getString(indexNombre) + "\n";
			}
		}
		return resultado;
	}
	...
	public String leerCanal() {
		...
		Cursor c = this.getReadableDatabase().query("canal", columna, null,
				null, null, null, null);
		int indexNombre = c.getColumnIndex(columna[0]);
		while (c.moveToNext()) {
